...
def load_data_2(path_edge, path_feat):
    """

    Args:
        path_edge: 存放边的路径，边的格式为 (0, 1) (2, 3)
        path_feat: 存放节点特征的路径，特征格式为 (500, 1, 0, 1, 0, 1, 0, 0, 0, 0) -- 500号节点的特征向量为 （1, 0, 1, 0, 1, 0, 0, 0, 0）
    ...
    """
    # load edges(type: List)
    print("loading " + path_edge)  # loading data/citeseer.txt
    G = nx.Graph()
    edge_list = []
    with open(path_edge, 'r') as f:
        for edge in f:
            edge = edge.strip('\n').split(' ')
            edge = [eval(x) for x in edge]
            edge_list.append((edge[0], edge[1]))  # 121→970
    G.add_edges_from(edge_list)
    adj = nx.adjacency_matrix(G)  # 得到邻接矩阵
    print(f"load data_edge of successfully")

    # load features(type: List)
    feat = []
    seq = []
    print("loading " + path_feat)  # loading data/citeseer/2/citeseer_feat_1.txt
    feat_table = {}
    count = 0
...
def normalize_adj(adj):  # 对称规范化邻接矩阵
